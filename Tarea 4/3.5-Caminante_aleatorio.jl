...
# ╔═╡ 55da1dff-0560-4bd3-99b0-31135df5d744
...
# ╔═╡ 00e98870-fc37-4b29-8fa6-31e1a5f10f29
rand()

# ╔═╡ 255a9909-6f09-485c-842a-42346ff32a14
rand(Int)

# ╔═╡ 62433e34-0d5b-4083-b49c-03733a77969e
rand(Bool)
...
# ╔═╡ f02c6d51-bdd8-4121-b9a0-8c1fd6c1abc7
a = rand()

# ╔═╡ 2918eae9-cac0-4dda-92eb-ef063bb2ad1b
b = rand()

# ╔═╡ 41a80720-ab01-4931-b86f-1c72cf48c993
histogram([aleatorioUniforme(a,b)], legend = false,title = "Aleatorio Uniforme")
...
function unPaso()                                    # definimos la función unpaso
	return 2*rand(Bool) -1                           # definomos a la función como dos veces rand de numerosBooleanos menos uno 
end	
...
function variosPasos(n)                             #Definimos la función como vrios pasos
	return 2*rand(Bool,n).-1                        # ocupamos la misma funcion que en la anterior pero usando un punto para que sea un arreglo
end
...
function caminataAleatoria(n)                        # definimos la funcion caminata aleatoria
	return variosPasos(n)                            # regresamos a la función varios pasos
	caminata = cumsum([0;pasos])                     # definimos a la caminata como la suma de los elementos
	return caminata                                  # regresamos a caminata
end
...
# ╔═╡ 0560ef30-3224-446a-9a27-c269ee2800a8
begin
	function graficaCaminata(caminataAleatoria)                #definimos la funcion
	n = length(caminataAleatoria)                           # definimos n como el tamaño de la funcion
	plot(1:n,caminataAleatoria,seriestype = :scatter,title= "Gráfica de caminata", xlabel = "Número de pasos", ylabel = "Posición")                                             # graficamos la funcion de 1 a n y definimos sus caracteristicas
end
n2 = 150                                                       # definimos n
caminataleatoria = caminataAleatoria(n2)                        # definimos la caminata aleatoria para ese n
graficaCaminata(caminataleatoria)                             
end
...
function graficaCaminata!(caminataleato)                      # definimos la funcion grafica caminata  
	n = length(caminataleato)                                 # definimos n como el tamaño de la caminata aleatoria
    plot!(1:n, caminataleato, seriestype = :path)             # graficamos de 1 a n la caminata aleatoria
end

# ╔═╡ 006d60fb-7d15-4c13-b571-2c3ed89748eb
begin
n = 500                                                # definimos nuestra n
for i in 1:5                                           # para un valor de i de1 a 5
	caminataleato = caminataAleatoria(n)               # definimos caminataleato = caminata Aleatoria para n
	graficaCaminata!(caminataleato)                    # graficaCaminata! para caminataleato
end
xlabel!("Número de pasos")                             
ylabel!("Posición")
end

# ╔═╡ b8a730f2-008a-474e-b615-2290393c646f
rand(n)

# ╔═╡ 2a9e4905-c059-4f4d-b9c0-04229611db30
rand(Int,n)

# ╔═╡ 2e79f6fe-0023-4627-a150-e901378fbd4c
rand(Bool,n)
...
anim = @animate for i in 1:n                           # generamos una animación para i de 1 a n
	graficaCaminata(caminatal[1:i])                    # usamos la función graficaCaminata para una caminata
end
	return anim                                        # regresamos a la función anmacion
end
n3= 220                                                # definimos una n
caminatal = caminataAleatoria(n3)                      # definimos la caminata
anima = animaCaminata(caminatal)                       # definimos la uncion anima
gif(anima, "caminatal.gif" , fps =20)                  # generamos un gif con la funcion anima
end
...
function caminataAleatoria2D(n)                                          # definimos la función 
	...
	x[i] = x[i-1] +dx                                  # xi es la x de i-1 mas dx
	y[i] = y[i-1] + dy                                 # yies la y de i-1 mas dy
	end
	return [x, y]                                      # regresamos a x y y 
	end

n4 = 1050                                                # definimos  una n
caminatalea = caminataAleatoria2D(n4)                    # definimos una caminata
plot(caminatalea[1], caminatalea[2], xlabel ="Posición horizontal", ylabel= "Posición vertical", title= "Caminante aleatorio")                                              # graficamos la cainata en 1 y en 2
end
...
# ╔═╡ 88f2e59e-234d-47ad-b32e-dbecc6d3308d
begin
...
N3 = 579                                                  # definimos una n
camino = caminataAleatoria2D(N3)                          # definimos camino como la caminata aleatoria en n
graficaCaminata2D(camino)                                 # usamos la uncion de grafica Caminata en 2d para camino
end
...
# ╔═╡ 319681d7-0bb1-4b15-afb1-40d2c9cd88ea
begin
function graficaCaminata2D!(Caminata,g)                       # definimos la funcion
	...
	n5= 7689                                                  # para un n
g = plot(title= "Cinco caminatas aleatorias", xlabel= "Coordenada x", ylabel = "Coordenada y", legend = false, aspect_ratio=1)                                 # definimos g como la grafica de las cinco caminatas

for i = 1:5                                       # para una i de 1 a 5
	Caminata = caminataAleatoria2D(n5)            # definimos caminata como la caminata aleatoria en 2D de n
	graficaCaminata2D!(Caminata,g)                # usamos la función grafica caminata para la caminata y g
end
	g
end
...
# ╔═╡ 05d11a3f-4a64-4a62-b2ad-4166eea84c37
begin
function animaCaminata2D(caminatal)                               # definimos la función
	animacion1 = plot(title="Caminata aleatoria en 2d", xlims=(-50,50), ylims= (-50,50))   # definimos animacion como la grafica de laa caminata aleatoria
		for i in 1:length(caminatal)        # para i en 1 hasta el tamaño de la caminata
		graficaCaminata2D(caminatal[1:i])   # usamos la graficacaminata2d  para el camino de 1 a i
	end
	return animacion1                        # regresamos a la animacion
end

function graficaCaminata2D(caminatal, animacion1)        # definimos la funcion grficaCaminata 2D para un camino y un animaciom
	x = cumsum(caminatal[1,:])                           # definimos x como la suma de los arreglos de caminata
	y = cumsum(caminatal[2,:])                            # definimos y como la suma de los arreglos de caminata
	plot!(animacion1, x, y, color=:green, lw=1.5)         # graficamos la animacion, x ,y 
	scatter!(animacion1, x[end], y[end], color=:red, ms=4)   # dibuja los puntos de la animacion1 y de el punto final de x y de y
end
	caminatal1 = [randn(2) for i in 1:100]                # definimos caminatal1 como numeros random para i de 1 a 100
	@gif for i in 1:length(caminatal1)                   # creamos un gif para i de 1 al tamaño de la caminata
		animaCaminata2D(caminatal1[1:i])                  # usamos animaCaminata2D para la caminatal
end
end
...
# ╔═╡ 1d640587-0351-4b48-bd42-c728d03b5e03
begin
function caminataAleatoria3D(n)                         # definimos la funcion camnata aleatoria en 3dimensiones
	caminota = [[0.0, 0.0, 0.0]]                       # definimos la caminata inicial                        
	for i in 2:n                                       # para una i de 2 a n
		paso = [randn(), randn(), randn()]              # definimos el paso como un vector de 3 entradas con valores diversos
		nuevo_punto = caminota[i-1] + paso             # definimos nuevo punto como la caminata de i-1 mas paso
		push!(caminota, nuevo_punto)                  # arregla caminota al final de la funcion nuevo punto
	end
...
n7 = 10780                                          # definimos n
caminota = caminataAleatoria3D(n)                   # definimos la caminota como la caminata aleatoria en n
graficaCaminata3D(caminota)                         # usamos la funcion grafica Caminata 3d para caminota
end
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "624948b2164fb5690a035a82c4d5a6f544d80767"
