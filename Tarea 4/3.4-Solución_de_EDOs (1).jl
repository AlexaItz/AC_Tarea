...
function tamañodePaso(t0,tf,N)            #Definimos la función tamaño de paso según lo conocemos
  h = (tf-t0)/N
	return h
end
...
function arregloUniforme(f,t0,tf,N,h)                               #definimos la función con f la funcion, t0 el tiempo inicial y tf el tiempo final, h el tamaño de paso
if N <= 0                                                           # si N no es un entero positivo se manda un mensaje de error
	error("Debe ser un entero positivo")
end	
	
	h = (tf-t0)/N                                                  # definimos el tamaño de paso
    x = collect(x0:h:xf)                                           # definimos x como una colección de x0 hasta xf
	y = zeros(n+1)                                                 # definimos y como un arreglo de ceros de n+1 
	y[1] = y0                                                      # definimos y1 = y0

for i in 1:n                                                      # para i en 1 a n
	y[i+1] = y[i] +h * f(x[i], y[i])                              # y[i+1] = y[i] +h *f(x[i],y[i])
end
	return(x,y)                                                   # regresamos x,y
end
...
function paso_euler(ti,xti,g,h)                     # definimos la función paso de euler con ti un valor de tiempo, xti, una aproximación de x, g es una función de x, h es el tamaño del arreglo uniforme
	xti1 = xti + h *g(ti,xti)                      # definimos xti1 = xti + h *g(ti,xti)
return xti1                                        # regresamos xti1
end
...
Estariamos realizando una aproximación  
...
# ╔═╡ 40713687-5ed6-42db-855d-67751e9eb17c
begin
	function euler(g,x0,t)                                   # definimos la funcion euler usando g la función de x y t, x0 que es la condición inicial, t el arreglo uniforme de tiempos
    n = length(t)                                       # n es el tamaño de t
	h = t[2] -t[1]                                      # h definido como la diferencia entre t2 y t1
	xt = zeros(n)                                       # xt es el arreglo de ceros de n
	xt[1] = t0                                         # definimos xt[1] =xt0

	for i in 1:n-1                                      # para i en 1 a n-1
		xt[i+1] =xt[i] +h * g(t[i], xt[i])              # xt[i+1] =xt[i] +h * g(t[i], xt[i])
	end
	 return xt                                          # regresamos xt
end
function m(l, x)                                        # ahora la definimos una funcion que solo depende de una variable
    return 2*l
end
t0 = 0.0                                                # el tiempo inicial
l = range(0.0, 1.0, step=0.1)                           # definimos la t
euler(m, t0, l)                                         # probamos la función
end
...
# ╔═╡ a431fa11-11cb-4c02-a2a0-c5a78306be29
@bind N Slider(0:100, default=0.5)           # definimos N como deslizador

# ╔═╡ 51d4f516-0774-4605-882f-9cb16960fee5
begin                                #definimos la función
	L= function K(j,k)
	return k
	end
end

# ╔═╡ 02f46ca4-2d88-4405-a812-eafe8bdfaa71
begin
	j0=0
	jf=3.0
	j= range(0.0, 3.0, step=0.1)                           
	hm= tamañodePaso(j0,jf,N)
	t_g=arregloUniforme(L,j0,jf,N,hm)
	xt_g= euler(K,j0,j)
end

# ╔═╡ 90d56437-9af0-432d-820c-94295be1e18a
scatter(t_g,xt_g, color="blue", legend=false)
