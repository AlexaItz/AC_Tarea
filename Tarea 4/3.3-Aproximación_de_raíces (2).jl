...
# ╔═╡ 86c66525-c534-4e69-a4c2-744bc12d390f
using Plots, PlutoUI
...
# ╔═╡ 71e61fb2-ac6c-41c4-9803-d04246cfd6a2
...
function newton(f::Function,f′,x0,n)               # definios la funcion con f  como la función, f' la derivada, x0 una                                                       aproximación inicial de una raíz de f y n el número de iteraciones
	x = x0                               # para x0 lo definimos como x
	i = 0                                # tomamos la primera iteración
	while abs(f(x)) > i < n              # para cuando el valor aboluto de la función en mayor que la iteracion y la                                            iteracion es menor que el número de iteraciones
	   x = x - f/f′                      # usamos la formula de las iteraciones e Newton
		i += 1                           # a la iteracion le sumamos uno
end
	return x                             # regresamos a la x
...
function newtonRecursiva(f,df,x0,t,n)         # Definimos la función newtonrecursiva con la funcion, la deivada, un valor inicial, variable t y n el numero de iteraciones
	if n == 0 || abs(f(x0)) < t                # Si n es igual a 0 o el valor absoluto de la funcion en x es menor a t
    return x0                                  # regresa a x0
	else
		x = x0 -f(x0) /df(x0)                  # en el otro caso definimos x =x0-f(x0)/df(x0)
	return newtonRecursiva(f,df,t,n-1)         # regresa a la función pero esta vez con n-1 iteraciones
	end
end
...
function newtonDistancia(f,f′,x0,y,epsilon)                                # Definimos la función newtonDistancia
n = length(x0)                                                             # definimos n como el tamaño de x0
 if n != length(y)                                                         # si el tamaño de x0 es diferente al tamaño de y
throw(DimensionMismatch("Las dimensiones de los vectores deben ser iguales"))   # arroja este letrero cuando las dimensiones de los vectores no sean iguales
 end
	d = 0.0                                       # definimos d como la distancia 0
	g = zeros(T, n)                               # definimos g como el arreglo de ceros T,n 
	h = zeros(T,n,n)                              # definimos h como el arreglo de ceros T,n.n
for i in 1:n                                      # para i en 1 a n
	g[i] = f(x0)[i] - y[i]                        # la función g(i) = f(x0)(i) -y(i)
	d += g[i]^2                                   # d and g(i)^2
end
	while true                                    # mientras verdadero sea verdadero
	for i in 1:n, j in 1:n                        # para i en 1 a n y j en 1 a n
	 h[i,j] = f′(x0,i)[j]                         # h(i,j)= la derivada de (x0,i)(j)
	end
  try                                             # cuando suceda qu z sea igual al inverso de h por g
	  z = -inv(h)*g
  catch ex                                        # sucede la excepción
  end
	if norm(z) < epsilon                          # si la norma de z es menor a epsilon
	 break                                        # aqui se interrumpe el loque while
	end
		d0 = d                                    # definimos d0 = d
   for i in 1:n                                   # para i dentro de 1 a n
	x0[i] += z[i]                                 # x0(i) and z(i)
    g[i] = f(x0)[i] -y[i]                         # definimos g(i) = f(x0)(i) -y(i)
	d += g[i]^2                                   # d and g(i)^2
   end
		if d >= d0                                # si d es menor o igual a d0
		return -1.0                               # regresa a -1
		end
	end
	return sqrt(d)                                # regresa a la raiz cuadrada de la distancia
end
...
derivadaNumérica = (f,x,h) -> (f(x+h)-f(x))/h               #definimos la derivada numérica
...
function newtonDn(f,x0,h,epsilon)                     # Definimos la funcion newtonDn
x= x0                                                 # sea x igual a x0
	while true                                        # mientras verdadero sea verdadero
		fx = f(x)                                     # fx lo definimos como la función en x
		derivadaNumérica = (f(x+h)-f(x))/h            
	if abs(derivadaNumérica) < 1e-9                   # si el valor absoluto de la derivada es menor a 1e-9
		return -1.0                                   # regresa a -1
	end
	 x -= fx/derivadaNumérica                         # resta binaria de x y fx/ la derivada 
		if abs(fx) < epsilon                          # si el valor absoluto de fx es menor a epsilon
		break                                         # para el ciclo while
		end
	end
	return x                                          # regresa a x
end
...
# ╔═╡ ac0495a8-c597-4d14-872f-204cd6847ac1
@bind x0 Slider(-10:0.1:10, default=0.1)          #definimos un deslizador para x0

# ╔═╡ 3efd83d2-e83b-4742-9e72-881c8e0ce80d
@bind h Slider(1000:1000:1000000, default=1000)       # definimos un deslizador para h

# ╔═╡ 1c590251-c7d1-4b1e-9350-07cc2b189744
@bind n Slider(1:1000)                             # definimos un deslizador para n

# ╔═╡ 005372a4-35cc-4d81-846e-0aa9b66a6ce5
f(x) = cos(x) # Luego, usa esta celda para reasignar `f`                     
...
# ╔═╡ 425100f4-a412-4bd4-9a7b-d806553211ec
begin
