...
Vy= r*sin(θ)
 ...
 k(x) = ((Vx)*x, altura+(Vy)*x - (9.8*x^2)/2)
kx(x)= (Vx)*x
ky(x)= altura+(Vy)*x	-(9.8*x^2)/2

   animacion= @animate for t in 0:0.1:5
...
scatter!([kx(t)],[ky(t)])
   ...
   gif(animacion, "Tiro_Parabólico.gif", fps =30)
end	
...
begin
	function parabola(a,r,θ,t)

	Vrx = r * cos(θ)
	Vry = r * sin(θ)

p(x) = ((Vrx) * x, a + (Vry) * x - ((9,8 * x^2)/2))
py(x) = (a + (Vry) * x-((9.8 * x^2)/2))
px(x) = (Vrx * x)

	tiro = @animate for t in 0:0.1:5
		plot(p.(range(-(3),0,step = 0.1).+t),legend = false, title = "Función de tiro parabólico", lw = 3, ls =:dot)

	scatter!([px(t)],[py(t)])

		x_max = (Vrx * ((2 * Vry/9.8 + ((2* 	Vry/9.8)^2 + 8 * a/9.8)^(1/2))/2))
	...
	gif(tiro, "Tiro_Parabólico_2.gif", fps =30)
	end
end
...
begin
 default(legend = false, title = "Superficie de la función h")
