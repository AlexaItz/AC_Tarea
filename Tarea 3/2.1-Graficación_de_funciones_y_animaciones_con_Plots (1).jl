...
# ╔═╡ cad7b8e6-0449-4e76-a34a-74b66db46cb1
@bind l3 Slider(0.05:0.05:2, default=0.05)

# ╔═╡ 3998ebd0-f380-47c2-b3a6-9d6c6e926b8d
l3

# ╔═╡ d5907a72-543c-46d6-b0f3-944b25d79837
begin
    plot(sin,-π:l3:π, title = "Funciones trigonométricas", xlabel = "x", ylabel = "y", color = "darkorange", label = "\$\\sin(x)\$" )     # Haz una gráfica tipo 'plot' de seno y luego
    plot!(cos,-π:l3:π, title = "Funciones trigonométricas", xlabel = "x", ylabel = "y", color = "blue", label = "\$\\cos(x)\$") # haz una gráfica tipo 'plot'de coseno encima
end
...
# ╔═╡ 5ad1c964-59e1-488a-a750-262f3e2b5796
begin
	altura= 5                 #Definimos las variables de altura y radio y ángulo
	r= 4
	θ= π/6      

Vx= r*cos(θ)                   #Definimos la componente del vector velocidad en x y en y
Vy= r*sin(θ)

 k(x) = ((Vx)*x, altura+(Vy)*x - (9.8*x^2)/2)                         #Definimos el vector posición, además de la componente x y y
kx(x)= (Vx)*x
ky(x)= altura+(Vy)*x	-(9.8*x^2)/2

   animacion= @animate for t in 0:0.1:5             # Para cada t en 0:0.1:5 se grafica la linea punteada del recorrido
	plot(k.(range(-(3),0,step=0.1).+t),
		legend = false, title = "Tiro parabólico", lw= 3, ls=:dot)

scatter!([kx(t)],[ky(t)])                               # Se gráfica la partícula

	x_max= Vx*((2*Vy/9.8+((2*Vy/9.8)^2+8*altura/9.8)^(1/2))/2)
	y_max= altura+(Vy)*(Vy/9.8)-9.8*(Vy/9.8)^2/2
	xlabel!("x")                    # Fijamos las variables de máximo alcance horizontal y vertical
	ylabel!("y")
	xlims!(0,x_max+0.5)
	ylims!(0,y_max+0.5)
  end

   gif(animacion, "Tiro_Parabólico.gif", fps =30)                      #Convertimos la variable animación en un gif
end	
...
# ╔═╡ dbbf0820-4781-4235-a3d0-b4a5e2bb7e51
begin
	function parabola(a,r,θ,t)                                       # Definimos la función parabola con las variables

	Vrx = r * cos(θ)                                         # definimos las componentes del vector velocidad
	Vry = r * sin(θ)

p(x) = ((Vrx) * x, a + (Vry) * x - ((9,8 * x^2)/2))                    # definimos las componentes y el vector posición
py(x) = (a + (Vry) * x-((9.8 * x^2)/2))
px(x) = (Vrx * x)

	tiro = @animate for t in 0:0.1:5          # Definimos la variable tiro que gráfica la línea y las características del plot
		plot(p.(range(-(3),0,step = 0.1).+t),legend = false, title = "Función de tiro parabólico", lw = 3, ls =:dot)

	scatter!([px(t)],[py(t)])                                           # Se grafica la particula

		x_max = (Vrx * ((2 * Vry/9.8 + ((2* 	Vry/9.8)^2 + 8 * a/9.8)^(1/2))/2))  # los valores de máximo alcance
		y_max = (a + (Vry) * (Vry/9.8) -9.8 * (Vry/9.8)^2/2)
	...
	gif(tiro, "Tiro_Parabólico_2.gif", fps =30)        # convertimos la variable tiro en un gif
	end
end
...
# ╔═╡ b3fbbb34-3901-45aa-a990-64f66bc77b65
begin
 default(legend = false, title = "Superficie de la función h")        # Colocamos el titulo de la gráfica
	x = y = range(-5, 5, length = 40)                                 # definimos la altura y el rango
	zs = zeros(0,40)                                                  # definimos n
	n = 100

@gif for i in range(0, stop = 2π, length = n)                         # Creamos un gif para algun i entre 0 y n 
	b(x,y) = cos(x + 10sin(i)) + sin(y)                              # definimos la función

	p = plot(x, y, b, st =:surface, color = :gist_rainbow)                # graficamos la superficie de la función con multicolor
	fixed_x = zeros(40)
	z = map(b, fixed_x, y)
